...
sentences in *Parsed CoNLL-U*. If `None`, then the loaded *test corpus* is
used. You can specify a ***Corpuscula***'s corpora wrapper here. In that case,
the `.test()` part will be used.

**save_to**: the name of the file where you want to save the results. Default
is `None`: we don't want to save.

Returns an iterator of **sentences** in *Parsed CoNLL-U* format with LEMMA
field filled.
